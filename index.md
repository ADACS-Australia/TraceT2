...
For quick, hands-on introduction, check out the [tutorial](tutorial.md).
...
* `FIELD_ENCRYPTION_KEY`: This key is used to encrypt passwords and API keys that must be stored in the database to be later used when making observatory requests. Changing this key will invalidate all models that currently store encrypted data. This key can be generated using `from cryptography.fernet import Fernet; key = Fernet.generate_key();`.
